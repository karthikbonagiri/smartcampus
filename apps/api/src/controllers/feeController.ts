...
export const getFees = async (req: Request, res: Response) => {
  ...
  const fees = await Fee.findAll({ where: { schoolId: user.schoolId } });
  res.json(fees);
};
...
export const getStudentFees = async (req: Request, res: Response) => {
  ...
  const fees = await Fee.findAll({
    where: { studentId: req.params.studentId, schoolId: user.schoolId },
  });
  res.json(fees);
};
...
export const updateFee = async (req: Request, res: Response) => {
  const user = req.user as any;
  const fee = await Fee.findOne({ where: { id: req.params.id, schoolId: user.schoolId } });
